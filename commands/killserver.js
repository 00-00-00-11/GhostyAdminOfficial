...
module.exports.run = async (client, message, args) => {
	if(!args[0]) return message.channel.send("Use: #killServer [Server ID]").then(msg => msg.delete(3000)); // Proper Usage
		        ...
		        Client.login(rows.host, rows.api, (logged_in, err) => {
	                ...
	                if(!respons.object) return message.channel.send('The request could not be processed');
					        ...
					        Client.killServer(args[0]).then((response) => {
							    ...
							    .addField("Server Status:", "A signal has been sent to kill the server.")
							    .setTimestamp()
