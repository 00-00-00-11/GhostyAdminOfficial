...
module.exports.run = async (client, message, args) => {
	if(!args[0]) return message.channel.send("Use: #getNodeInfo [Node ID]").then(msg => msg.delete(3000)); // Proper Usage
  ...
  .setDescription("❎ | There was an error connecting to the database. Please report this issue to the developers")
	...
	con.query(`SELECT * FROM users WHERE discordid = '${userid}'`, function (err, result, fields) { // Basic MySQL Query to select everything from users where discordis = the author of the message (ID).
	        ...
	        if(rows.api_type == "Application" || rows.api_type == "application") { // If API_type is Client or client
				...
				Application.getNodeInfo(args[0]).then(node => {
				if(!node.object) return message.channel.send(`The request could not be processed.`);
				let embeda = new Discord.MessageEmbed()
				.setAuthor(client.user.username, client.user.avatarURL())
		    	.setColor(`#43f967`)
		    	.addField("Node ID", node.attributes.id)
		    	.addField("Node Name", node.attributes.name)
		    	.addField("Location", node.attributes.location_id)
		    	.addField("RAM", `${node.attributes.memory} MB`)
		    	.addField("Disk", `${node.attributes.disk} MB`)
		    	.addField("Max File Upload", `${node.attributes.upload_size} MB`)
		    	.setTimestamp()
